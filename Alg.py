...
min_qty = [0] * N          # теперь можно не покупать товар
max_qty = [random.randint(5, 20) for _ in range(N)]

# Минимальная и максимальная возможная общая стоимость
min_total = sum(prices[i] * min_qty[i] for i in range(N))  # всегда 0
...
class Application(tk.Tk):
    def __init__(self):
        ...
        ttk.Label(info_frame, text=f"Минимальная возможная сумма (ничего не покупать): 0 руб.").pack(side=tk.LEFT, padx=10)
